...
class TokenBaseEmbedding(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if kfg.G_TOKENS_IDS in batched_inputs:
            ...
            # g_emo_ids = batched_inputs[kfg.G_EMOTION_IDS]
            # g_emo_type = batched_inputs.get(kfg.G_EMO_TYPE, None)
            g_token_embed = self._forward(g_tokens_ids, token_type_ids=g_tokens_type, time_step=time_step)
            # g_emo_embed = self._forward(g_emo_ids, token_type_ids=g_emo_type, time_step=time_step)
            ret.update({
                kfg.G_TOKEN_EMBED: g_token_embed,
                # kfg.G_EMO_EMBED: g_emo_embed
                })

        if kfg.G_EMO_WORD_IDS in batched_inputs:
            g_emo_word_ids = batched_inputs[kfg.G_EMO_WORD_IDS]
            g_emo_word_embed = self.embeddings(g_emo_word_ids)
            ret.update({
        ...
        if kfg.PRED_EMBED not in batched_inputs:
            pred = batched_inputs[kfg.CONCEPTS_PRED]
            pred_embed = self.embeddings(pred)
