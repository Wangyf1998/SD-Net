...
__all__ = ["ArtemisEncoder"]


@ENCODER_REGISTRY.register()
class ArtemisEncoder(nn.Module):
    @configurable
    def __init__(
            self,
            *,
    ...
    def forward(self, batched_inputs, mode=None):
        ...
        if mode == None or mode == 'v':
            ...
            concepts_pred_ids = batched_inputs[kfg.CONCEPTS_PRED_IDS]
            concept_mask = batched_inputs[kfg.ATTR_MASK]
            concepts_embed = self.embeddings(concepts_pred_ids)
            #TODO: 注意这里的embeddings是把concept编码成num_classes的，如果使用art_style进一步分化concept_pool，
            ...
            slot_mask = torch.ones((semantics_embed.shape[0], slot_embed.shape[1]), device=slot_embed.device).to(
